...
// app/modules/publicStudy/components/TaskPostSurveyClient.tsx
//
// Purpose:
// - Load TASK post survey via /api/study/task/:n/post
// - Submit via /api/study/task/:n/post/submit
//
// Why:
// - Standardized post-task measurement (trust/confidence/...) and clean DB persistence.
//
// Additional behavior (timing correctness):
// - Send periodic heartbeats while the survey page is open.
...
export default function TaskPostSurveyClient(props: { accessCode: string; taskNumber: string }) {
    const router = useRouter();
    const session = useRequireStudySession(props.accessCode);

    const [loading, setLoading] = useState(true);
    const [submitting, setSubmitting] = useState(false);
    const [title, setTitle] = useState(`Task ${props.taskNumber} - Post Survey`);
    ...
    // Heartbeat loop while post-survey page is mounted.
    useEffect(() => {
        if (!session) return;
    ...
    useEffect(() => {
        ...
        (async () => {
            setLoading(true);
            const res = await loadSurvey(session.token, `/api/study/task/${props.taskNumber}/post`);
            setLoading(false);
    ...
    async function submitConfirmed(answers: SubmitSurveyAnswer[]) {
        ...
        setSubmitting(true);
        const res = await submitSurvey(session.token, `/api/study/task/${props.taskNumber}/post/submit`, {answers});
        setSubmitting(false);
    ...
    async function onSubmit(answers: SubmitSurveyAnswer[]) {
        ...
        // Mandatory confirmation step (requested):
        // - No change to validation: if SurveyRenderer calls us, it already passed.
        // - No risk to session/token logic: we only delay the submit by one explicit user decision.
    ...
    return (
        <>
            ...
            <SurveyRenderer title={title} questions={questions as any} submitting={submitting}
