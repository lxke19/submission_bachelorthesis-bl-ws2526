...
// app/modules/publicStudy/components/PreSurveyClient.tsx
//
// Purpose:
// - Load PRE survey via /api/study/pre
// - Submit via /api/study/pre/submit
//
// Why:
// - Keeps /study/[accessCode]/pre/page.tsx very small.
//
// Additional behavior (timing correctness):
// - Send periodic heartbeats while the pre-survey page is open.
...
export default function PreSurveyClient({accessCode}: { accessCode: string }) {
    const router = useRouter();
    const session = useRequireStudySession(accessCode);

    const [loading, setLoading] = useState(true);
    const [submitting, setSubmitting] = useState(false);
    const [title, setTitle] = useState("Pre Survey");
    ...
    // Heartbeat loop while pre-survey page is mounted.
    useEffect(() => {
        if (!session) return;
    ...
    useEffect(() => {
        ...
        (async () => {
            setLoading(true);
            const res = await loadSurvey(session.token, "/api/study/pre");
            setLoading(false);
    ...
    async function submitConfirmed(answers: SubmitSurveyAnswer[]) {
        ...
        setSubmitting(true);
        const res = await submitSurvey(session.token, "/api/study/pre/submit", {answers});
        setSubmitting(false);
    ...
    if (loading) {
        return (
            <div className="py-10 space-y-2">
                <h1 className="text-2xl font-semibold text-slate-50">Pre Survey</h1>
